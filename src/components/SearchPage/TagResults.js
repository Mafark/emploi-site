                    ...
                    <div className="small-12 columns">
                        {
                            this.selectedTags.map((tag, index) => {
                                return (
                                    <div key={index} onClick={this.deleteSelectedTag.bind(this, tag)}
                                         className="tag activated circle small-bg">{tag}<i className="material-icons">close</i>
                                    </div>
                                )
                            })
                        }
                        {
                            this.props.state.search.searchTags.map((tag, index) => {
                                return (
                                    <div key={index} onClick={this.addSelectedTag.bind(this, tag)}
                                         className="tag disabled circle small-bg">{tag}</div>
                                )
                            })
                        }
                    </div>
